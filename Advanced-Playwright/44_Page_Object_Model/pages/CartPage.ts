...
export class CartPage {
  ...
  async getProductNames() {
    ...
    const productNames =
      await this.productNameCells.all();
    ...
    for (const product of productNames) {
      ...
      const name =
        (await product.textContent())?.trim() || '';
  ...
  async getProductPrices() {
    ...
    const priceElements =
      await this.productPriceCells.all();
    ...
    for (const price of priceElements) {
      ...
      const priceText =
        (await price.textContent())?.trim() || '0';
  ...
  async removeProductFromCart(productName: string) {
    ...
    for (const { row, deleteButton } of matchingRows) {

      await deleteButton.click({ force: true });

      // Wait until this particular product row
  ...
  async getTotalCartValue() {
    ...
    const totalText =
      await this.totalPrice.textContent();
