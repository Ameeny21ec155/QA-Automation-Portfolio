...
test.fail(): Informs Playwright that the test is expected to fail. If the test actually passes, the
...
// Suppose we ten test cases then we need execute 10 then we get error
//  one so we can skip that particular one test case to execute remaining.
...
test.fail('Fail Example -Using test.fail()',async({page})=>{
    ...
    // Incorect assertion (expected failure)
...
//5. test.only(): Ignores alm test and execute particular test only
