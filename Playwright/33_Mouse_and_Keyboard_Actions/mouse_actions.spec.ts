...
test('1.Toggle button changes state when clicked',async({page})=>{
    ...
    await toggleButton.click(); // locator based // Preferred
...
test ('2.Right click/context click',async({page})=>{
    ...
    const rightClickButton = await page.locator("button",{hasText:'Right Click Me'});

    // Right click action / context click --> opens the context menu
    await rightClickButton.click({button:'right'}); // this will perform the right click action. and other option left click(default),middle click (optional)
    ...
    //Handle dialog is optional , but if you want to verify the dialog message , you can uncomment the 
...
test('3.Mouse Hover interaction & tooltip',async({page})=>{
    ...
    console.log("attribut value:",await hoverButton.getAttribute('title'))
...
test('Copy Text button transfers value from field1 to field2',async({page})=>{
    ...
    await copyTextButton.dblclick(); // double click button
    await page.waitForTimeout(3000);
...
test('6. Drag and drop the draggable element into the drop zone',async({page})=>{
    ...
    await sourceItem.dragTo(targetItem) // Darg drop event triggers Dialog
...
test('Range slider' , async({page})=>{
    ...
    await page.keyboard.press('Home') // go to intial point '0'
